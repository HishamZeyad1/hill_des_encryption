...
class DesCipher extends StatefulWidget {
  @override
  _DesCipherState createState() => _DesCipherState();
}

const _string =
    "Java, android, ios, get the same result by DES encryption and decryption.";

class _DesCipherState extends State<DesCipher>with Helpers{
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () async {
            ...
            CollectionReference noteRef=FirebaseFirestore.instance.collection("message");
                await noteRef.add({"senderId":FirebaseAuth.instance.currentUser!.uid,"text":_text,
                  "textencrption":_encrypt,"encryptHex":_encryptHex,"created_at":created_at}).then((value){
                    print("success");
                    showDialog(context: context, builder: (context) {
                      return AlertDialog(icon:Icon( Icons.check,color: Colors.green),title: Text("The Note has been added",),);
                    },);
                    showSnackBar(context: context, message: "The Note has been added",error: false);
                    print("success1");
                }).catchError((onError){showSnackBar(context: context, message: onError.toString(),error: true);});
                  print("okey");
                },
          child: Icon(
            Icons.send,
